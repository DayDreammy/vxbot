...
class WeChatArticleHandler(MessageHandler):
    ...
    def __init__(self, db_path: str = 'data/message_monitor.db',
        ...
        self.unified_db_path = Path(
            '/home/yy/project/liujing-project-zhengquan/data/unified_information.db')
                        ...
                        if img_url:
                            ...
                            try:
                                ...
                                if response.ok:
                                    ...
                                    self.logger.error(
                                        f"下载图片失败: {img_url}, 状态码: {response.status}")
                            except Exception as e:
                                self.logger.error(
                                    f"下载图片失败: {img_url}, 错误: {e}")
            ...
            for i in range(0, len(articles), batch_size):
                ...
                for article in batch:
                    task = asyncio.create_task(
                        self._process_article_content(article, context))
                    tasks.append(task)
    ...
    async def store_to_unified_db(self, article: ArticleInfo, from_user: str) -> Optional[int]:
        ...
        if IF_ACCOUNT_WHITELIST:
            if from_user not in config_manager.account_industry_mapping:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 从行业映射获取行业信息
                industries = config_manager.account_industry_mapping.get(
                    from_user, [])
                ...
                if "database is locked" in str(e) or "cannot commit" in str(e):
                    if attempt < max_retries - 1:
                        self.logger.warning(
                            f"数据库访问冲突，正在重试 ({attempt + 1}/{max_retries})")
        ...
        try:
            ...
            if await self.fetch_article_content(article):
                ...
                async with aiosqlite.connect(self.db_path) as db:
                    ...
                    (title, url, content, pdf_path, images, account_name, processed, process_time)
                    ...
                    await db.commit()
                self.logger.info(
                    f"文章内容处理完成: {article.title}，from: {article.account_name}")
